    ...
    async (req: AuthRequest, res) => {
        try {
            ...
            if (!["text", "voice"].includes(type)) {
                return res
                    .status(400)
                    .json({ message: "Invalid channel type" });
            }
            ...
            if (server.owner.toString() !== req.user!._id.toString()) {
                return res
                    .status(403)
                    .json({ message: "Only server owner can create channels" });
            }
    ...
    async (req: AuthRequest, res) => {
        try {
            ...
            if (server.owner.toString() !== req.user!._id.toString()) {
                return res
                    .status(403)
                    .json({ message: "Only server owner can delete channels" });
            }
    ...
    async (req: AuthRequest, res) => {
        try {
            ...
            if (server.owner.toString() === req.user!._id.toString()) {
                return res
                    .status(400)
                    .json({ message: "Owner cannot leave server" });
            }
