...
router.get('/server/:serverId/:channelId', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    const channel = server.channels.find(ch => ch._id?.toString() === channelId);
    if (!channel) {
...
router.post('/server/:serverId/:channelId', authenticate, async (req: AuthRequest, res) => {
  try {
    const { serverId, channelId } = req.params;
...
router.post('/dm/:friendId', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    if (!friend || !req.user!.friends.includes(new Types.ObjectId(friendId))) {
