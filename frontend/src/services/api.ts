...
import { User, Server, Message, DirectMessage, FriendRequest, Channel } from '../types';
...
export const getFriends = async (): Promise<{ friends: User[]; friendRequests: FriendRequest[] }> => {
...
export const acceptFriendRequest = async (requestId: string): Promise<{ friends: User[]; friendRequests: FriendRequest[] }> => {
...
export const createChannel = async (serverId: string, name: string, type: string): Promise<Channel> => {
  const response = await api.post(`/servers/${serverId}/channels`, { name, type });
  return response.data;
};

export const getMessages = async (channelId: string, channelType: string, serverId?: string): Promise<Message[]> => {
  const url = channelType === 'server' 
...
export const sendDirectMessage = async (friendId: string, content: string): Promise<Message> => {
